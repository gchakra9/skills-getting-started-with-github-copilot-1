document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = participants.length
          ? `<div class="participants-list">${participants
              .map(
                (participant) => `
                  <div class="participant-row" data-activity="${name}" data-email="${participant}">
                    <span class="participant-email">${participant}</span>
                    <button class="participant-remove-btn" type="button" aria-label="Remove ${participant}" title="Remove ${participant}">✕</button>
                  </div>
                `
              )
              .join("")}</div>`
  ...
  activitiesList.addEventListener("click", (event) => {
    ...
    const participantRow = removeButton.closest(".participant-row");
    if (!participantRow) {
      return;
    }

    const activityName = participantRow.dataset.activity;
